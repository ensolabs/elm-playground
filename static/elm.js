...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.aH, request.I, true);
		} catch (e) {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {X: fragment, _: host, ad: path, af: port_, ai: protocol, M: query};
	});
...
var $author$project$Main$Model = F4(
	function (key, url, compilation, srcCode) {
		return {C: compilation, aa: key, O: srcCode, I: url};
	});
var $author$project$Main$GotCompilationResult = function (a) {
	return {$: 2, a: a};
};
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{ax: false, S: r.S, aC: r.aC, Y: r.Y, aH: r.aH, aR: r.aR, ar: r.ar, I: r.I}));
};
var $elm$http$Http$post = function (r) {
	return $elm$http$Http$request(
		{S: r.S, aC: r.aC, Y: _List_Nil, aH: 'POST', aR: $elm$core$Maybe$Nothing, ar: $elm$core$Maybe$Nothing, I: r.I});
};
var $elm$http$Http$stringBody = _Http_pair;
var $author$project$Main$compileSrcCommand = function (src) {
	return $elm$http$Http$post(
		{
			S: A2($elm$http$Http$stringBody, 'text/plain;charset=utf-8', src),
			aC: A2(
				$elm$http$Http$expectStringResponse,
				$author$project$Main$GotCompilationResult,
				function (res) {
					switch (res.$) {
						case 4:
							var body = res.b;
						...
						case 3:
							var body = res.b;
							return $elm$core$Result$Err(body);
						default:
...
var $author$project$Exercises$exerciseMap = $elm$core$Dict$fromList(
	A2(
		$elm$core$List$map,
		function (el) {
			return _Utils_Tuple2(el.b, el.a);
		},
		$author$project$Exercises$exercises));
...
var $author$project$Exercises$getExerciseSourceCodeOrDefault = function (maybeId) {
	return A2(
		$elm$core$Maybe$withDefault,
		$author$project$Exercises$defaultExercise,
		A2(
			$elm$core$Maybe$andThen,
			function (id) {
				return A2($elm$core$Dict$get, id, $author$project$Exercises$exerciseMap);
			},
			maybeId));
};
var $author$project$Main$init = F3(
	function (_v0, url, key) {
		var srcCode = $author$project$Exercises$getExerciseSourceCodeOrDefault(url.M);
		return _Utils_Tuple2(
			A4($author$project$Main$Model, key, url, $author$project$Main$Loading, srcCode),
			$author$project$Main$compileSrcCommand(srcCode));
	});
...
var $author$project$Main$reCompileSrc = F2(
	function (src, model) {
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{C: $author$project$Main$Loading, O: src}),
			$author$project$Main$compileSrcCommand(src));
	});
		...
		switch (msg.$) {
			...
			case 1:
				...
				var newModel = _Utils_update(
					...
					{I: url});
				return A2(
					$author$project$Main$reCompileSrc,
					$author$project$Exercises$getExerciseSourceCodeOrDefault(url.M),
					newModel);
			case 2:
				var res = msg.a;
				if (!res.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								C: $author$project$Main$Success(code)
							}),
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								C: $author$project$Main$Failure(err)
							}),
						$elm$core$Platform$Cmd$none);
				}
			default:
				var src = msg.a;
				return A2($author$project$Main$reCompileSrc, src, model);
		}
	});
...
var $author$project$Main$EditSrc = function (a) {
	return {$: 3, a: a};
};
...
var $author$project$Main$viewEditor = function (srcCode) {
	return A2(
		$elm$html$Html$textarea,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('rounded-sm border-2 m-2 mr-1 p-2 focus:outline-none flex-1 font-mono resize-none'),
				$elm$html$Html$Attributes$value(srcCode),
				$elm$html$Html$Events$onInput($author$project$Main$EditSrc)
			]),
		_List_Nil);
};
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							_Utils_Tuple2('hover:underline hover:font-bold', true),
							_Utils_Tuple2('text-blue-600 font-bold', active)
						]))
				]),
			_List_fromArray(
				[
...
var $author$project$Main$viewExercices = function (activeExercise) {
	return A2(
		$elm$html$Html$aside,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('w-64 p-4 border-r overflow-y-auto')
			]),
		_List_fromArray(
			[
				A2(
						...
						return A2(
							...
							_List_fromArray(
								[
									...
									_Utils_eq(
										activeExercise,
										$elm$core$Maybe$Just(exercise.b)))
								]));
					},
					$author$project$Exercises$exercises))
			]));
...
var $author$project$Main$viewParagraph = function (content) {
	return A2(
		$elm$html$Html$p,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('max-w-prose text-center')
			]),
		content);
};
var $author$project$Main$viewHeader = A2(
	...
	_List_fromArray(
		[
			$elm$html$Html$Attributes$class('w-full flex flex-col items-center justify-center p-8 shrink-0 space-y-4')
		]),
	_List_fromArray(
		[
			A2(
			...
			_List_fromArray(
				[
					$elm$html$Html$text('Learn Elm with Ensō')
				])),
			$author$project$Main$viewParagraph(
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('The exercies where originally made by '),
					A3($author$project$Main$viewLink, 'jgrenat', 'https://github.com/jgrenat', false),
					$elm$html$Html$text(' as a '),
					A3($author$project$Main$viewLink, 'workshop', 'https://github.com/jgrenat/compiler-driven-development', false),
					$elm$html$Html$text(', and are used with his kind permission.')
				]))
		]));
var $elm$html$Html$iframe = _VirtualDom_node('iframe');
var $elm$html$Html$Attributes$srcdoc = $elm$html$Html$Attributes$stringProperty('srcdoc');
var $author$project$Main$wrapErr = function (err) {
	return '<pre style="color:red; padding:1rem">' + (err + '</pre>');
};
var $author$project$Main$viewIFrame = function (compilation) {
	var _v0 = function () {
		switch (compilation.$) {
			case 0:
				return _Utils_Tuple2('Loading...', true);
			case 1:
				var err = compilation.a;
				return _Utils_Tuple2(
					$author$project$Main$wrapErr(err),
					false);
			default:
				var res = compilation.a;
				return _Utils_Tuple2(res, false);
		}
	}();
	var srcDoc = _v0.a;
	var loading = _v0.b;
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						_Utils_Tuple2('rounded-sm border-2 m-2 mr-1 p-2 focus:outline-none flex-1 font-mono resize-none transition transition-duration-1000', true),
						_Utils_Tuple2('blur-[2px]', loading)
					])),
				$elm$html$Html$Attributes$srcdoc(srcDoc)
			]),
		_List_Nil);
};
var $elm$html$Html$div = _VirtualDom_node('div');
var $author$project$Main$viewWrappable = function (content) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('flex flex-wrap flex-1')
			]),
		content);
};
	...
	return {
		S: _List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('min-h-screen flex flex-col bg-[#fff6f6]')
					]),
				_List_fromArray(
					[
						$author$project$Main$viewHeader,
						$author$project$Main$viewWrappable(
						_List_fromArray(
							[
								$author$project$Main$viewExercices(model.I.M),
								$author$project$Main$viewEditor(model.O),
								$author$project$Main$viewIFrame(model.C)
							]))
					]))
			]),
		aS: 'Ensō Elm Playground'
	};
};
var $author$project$Main$main = $elm$browser$Browser$application(
	{aG: $author$project$Main$init, aI: $author$project$Main$UrlChanged, aJ: $author$project$Main$LinkClicked, aQ: $author$project$Main$subscriptions, aT: $author$project$Main$update, aU: $author$project$Main$view});
